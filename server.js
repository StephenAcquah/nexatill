...
app.get('/api/users', requireAuth, requireStaffAdmin, async (req, res, next) => { try { const result = await pool.query(`${userSelect} WHERE tenant_id=$1 ORDER BY created_at`, [req.user.tenant_id]); res.json({ users: result.rows }); } catch (error) { next(error); } });
